...
def cmd(sock, do, what = '', where = '', at = ''):
    ...
    # Clear any pending data in socket buffer before sending
    sock.setblocking(False)
    try:
        while True:
            old_data = sock.recv(1024)
            if not old_data:
                break
    except:
        pass
    ...
    try:
        ...
        while True:
            if time.time() - start_time > 5.0:  # Increased to 5 second timeout
                print(f'\nERROR: Timeout waiting for response (buffer: {response_buffer[:50]}...)')
                return None
            
            # Use non-blocking recv with smaller chunks
            try:
                chunk = sock.recv(256).decode()  # Smaller recv buffer (256 instead of 1024)
            except:
        ...
        # Small delay after receiving to let robot process
        time.sleep(0.05)
...
try:
    while True:
        ...
        if iteration_count % 10 == 0:
            ...
            print(f"Socket stats: cmd_no={cmd_no}, connected={check_connection(sock)}")
        
        # Periodic keepalive - stop and small delay every 10 iterations
        if iteration_count % 8 == 0 and iteration_count > 0:  # Changed from 10 to 8 - reconnect BEFORE robot limit
            print("[Periodic maintenance: stop and reconnect]")
            try:
                cmd(sock, 'stop')
            except:
                pass
            ...
            try:
                sock.close()
                print("[Closed socket, reconnecting...]", end=' ')
            except:
                print("[Socket already closed, creating new...]", end=' ')
            
            time.sleep(0.3)  # Longer delay before reconnect
            ...
            try:
                ...
                time.sleep(0.2)  # Let robot stabilize after reconnect
            except Exception as e:
        ...
        # Check if we're approaching the problematic iteration
        if iteration_count == 13:
            print(f"\n!!! Reached iteration 13 - watching for issues !!!")
            print("[Diagnostic] Sending test stop command...")
            test_result = cmd(sock, 'stop')
            print(f"[Diagnostic] Test result: {test_result}")
            time.sleep(0.3)
        ...
        # CRITICAL: Add delay to prevent robot buffer overflow
        # Robot needs time to process commands and clear its buffer
        # Robot appears to close connection after ~10 commands, so we reconnect every 8
        time.sleep(0.4)  # Increased from 0.3 to 0.4 seconds between commands
